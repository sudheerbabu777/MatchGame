...
class MatchGame extends Component {
  ...
  renderWindowPage = () => {
    ...
    const filterTabs = imagesList.filter(each => each.category === category)
    return (
      <div className="container">
        <img src={image} alt="match" className="image_url" />
        <div>
          <ul className="tabsList">
            {tabsList.map(each => (
              <li className="-tab-item" key={each.tabId}>
                <button
                  type="button"
                  className={`button-tab ${
                    category === each.tabId ? 'active' : ''
                  }`}
                  onClick={() => this.onClickTabs(each.tabId)}
                >
                  {each.displayText}
                </button>
              </li>
  ...
  renderWinOrLose = () => {
    ...
    return (
      <div className="result-container">
        ...
        <button
          className="button-play-agin"
          type="button"
          onClick={this.onClickPlayAgin}
        >
          <img
  ...
  render() {
    const {isTrue, score, timer} = this.state
    ...
    return (
      <div>
        ...
        <div className="bg-container">
          {isTrue ? this.renderWinOrLose() : this.renderWindowPage()}
        </div>
      </div>
    )
  }
